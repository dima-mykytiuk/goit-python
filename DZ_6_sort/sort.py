...
def move_files(path, item_name, file_type, folder_name, base_path_to):
    if file_type in files_type.get(folder_name) and not os.path.exists(str(path) + folder_name + folder_sep + normalize(item_name.removesuffix('.'+file_type)) + '.' + file_type):
        FROM = str(path) + folder_sep + item_name
        TO = str(base_path_to) + folder_sep + folder_name + folder_sep + normalize(item_name.removesuffix('.'+file_type)) + '.' + file_type
        shutil.move(FROM, TO)
        ...
        if item_name not in folder_list:
            if os.path.isfile(path + folder_sep + item_name) and item_name != '.DS_Store':
                ...
                for folder_name in folder_list:
                    ...
                    elif file_type in files_type.get('archives') and not os.path.exists(str(path) + '/archives/' + normalize(item_name.removesuffix('.'+file_type))):
                        shutil.unpack_archive(str(path) + folder_sep + item_name, str(main_path) + '/archives/' + normalize(item_name.removesuffix('.'+file_type)))
            ...
            if os.path.isfile(path + folder_sep + folder_item) and folder_item != '.DS_Store':
                ...
                if file_type not in files_type.keys():
                    shutil.move(str(path) + folder_sep + folder_item,
                                str(path) + folder_sep + normalize(folder_item.removesuffix('.' + file_type)) + '.' + file_type)
...
def remove_empty_folders(path_abs):
    ...
    for path, folders, folders_items in walk[::-1]:
        ...
        if len(os.listdir(path)) == 0 and folder_name not in folder_list and path == str(_dir) + folder_sep + folder_name or len(os.listdir(path)) == 1 and '.DS_Store' in folders_items and folder_name not in folder_list and path == str(_dir) + folder_sep + folder_name:
            shutil.rmtree(path)
